...
class ExamIntelligencePipeline:
    ...
    async def run(self, files_data: List[Dict[str, Any]], time_available: str) -> dict:
        """
        Executes the optimized exam intelligence pipeline.
        ...
        # 1. Content Generator + Revision Optimizer
        # 2. Question Engine + Answer Engine
        print("[Exam Intel] Running Modules 3-7 in Parallel...")
        content_revision_task = self._step_content_generator_and_revision_optimizer(notes_text, top_topic_names, time_available)
        question_answer_task = self._step_question_and_answer_engine(pyq_text, qb_text, top_topic_names, notes_text)

        content_revision_result, question_answer_result = await asyncio.gather(
        ...
        return {
            ...
            "exam_notes": content_revision_result.get("exam_notes", []),
            "important_questions": question_answer_result.get("important_questions", []),
            "predicted_questions": question_answer_result.get("predicted_questions", []),
            "model_answers": question_answer_result.get("model_answers", []),
            "quick_revision": content_revision_result.get("quick_revision", []),
            "last_day_revision": content_revision_result.get("last_day_revision", []),
            "study_plan": content_revision_result.get("study_plan", []),
...
Aim for 3-5 MUST DO, 4-6 SHOULD DO, and group the rest in LOW PRIORITY."""
    ...
    async def _step_content_generator_and_revision_optimizer(self, notes_text: str, top_topics: List[str], time_available: str) -> dict:
        if not top_topics:
            return {
                "exam_notes": [],
                "quick_revision": [],
        ...
        sys_prompt = """You are an expert academic content generator and study planner.
Generate 'Smart Notes' ONLY for the provided high-priority topics based on the syllabus context, and create a highly optimized time-based study plan and revision sheet based on the 80/20 rule.

Return ONLY a strictly formatted JSON matching this schema:
{
  "exam_notes": [
    {
      "topic": "Topic Name",
  ...
  "study_plan": [
    {"day_or_block": "Day 1 / Morning", "topic": "Name", "hours_allocated": "2h", "focus": "PYQ solving"}
        ...
        return await generate_json_response(sys_prompt, prompt, model=self.model, max_tokens=3500)

    async def _step_question_and_answer_engine(self, pyq_text: str, qb_text: str, top_topics: List[str], notes_text: str) -> dict:
        sys_prompt = """You are a UNIVERSITY GOLD MEDALIST and EXAM TOPPER. 
Analyze past patterns and question banks. Predict questions for the upcoming exam.
Then, write COMPLETE, EXAM-READY model answers for the top 3 most important questions that students can memorize and write AS-IS to score full marks.

CRITICAL RULES FOR MODEL ANSWERS (Write exactly 3 model answers):
- Structure each answer as multiple clear paragraphs separated using "\\n\\n" (double newline).
- 2-5 marks: crisp definition + explanation + example.
- 10+ marks: full coverage, 4-5 paragraphs including opening definition, detailed explanation, real examples with names, and solid conclusion.
- Write in FIRST PERSON as a confident student writing in an exam.
- Include key terms/definitions examiners look for.
...
Include at least 5 important and 3 predicted questions in their respective lists, and write model answers for the top 3 important questions."""
        
        prompt = f"Target Topics: {', '.join(top_topics)}\n\nPYQs:\n{pyq_text[:15000]}\n\nQ-Bank:\n{qb_text[:15000]}\n\nReference Material for answers:\n{notes_text[:15000]}"
        return await generate_json_response(sys_prompt, prompt, model=self.model, max_tokens=4000)
